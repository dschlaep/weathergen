    ...
    if(toupper(vars[iv]) == "PRCP"){
      for(is in 1:num_site){
        ...
        if (any(toupper(spells)=="EXTREME")) { # Currently, if extreme is in spells, ignore it. Implementation will follow
          ...
          temp <- wgen_out_array[[is,1]]$obs[['PRCP']]  > dw_threshhold # get dry 
          temp2[temp2==1] <- temp[temp2==1]          
          Spells[1 + k1, 1 + is, , ] <- t(aggregate(temp2, by=list(month), FUN=function(x) {
            temp <- rle(x)
            res <- sapply(list(temp$lengths[temp$values==0], temp$lengths[temp$values==1], temp$lengths[temp$values==2]), FUN=function(x) c(mean(x), sd(x), psych::skew(x)))
            return(res)})[, 2])
